...
```{theorem}
Let $X$ and $Z$ be random vectors of dimensions $\dimx$ and $\dimx^\prime$ respectively. If $\varphi\colon\R^{\dimx}\times\R^{\dimx^\prime}\to\R$ is a measurable function, then there exists a Borel set $A^*\subseteq\R^\dimx$ such that 
\begin{equation}
...
$$
where, for each $x\in\R^\dimx,$ $B_x := \{z\in \R^{\dimx^\prime}\colon\,\varphi(x,z)\in B\}.$

```

```{example}
With the same notation as in the above theorem, assume the function $\varphi$ does not depend on its second argument, that is, for some $\psi\colon\R^\dimx\to\R$ it holds that $\varphi(x,z) = \psi(x)$ for all $x\in\R^\dimx$ and $z\in\R^{\dimx^\prime}.$ Then an easy check tells us that
\begin{equation}
...
$$
are both non-decrasing functions, which can only happen if $\beta_1$ is constant-in-$\tau.$ This example can be generalized to the case $\dimx>2.$ 
...
```{remark}
To generalize Proposition \@ref(prp:hyperplanes) to dimensions $\dimx>3$ one needs to specify $\dimx$ functions $v_d\colon(0,1)\to\R^\dimx,$ $d\in\{1,\dots,\dimx\}$ that are non-decreasing and left-continuous. These correspond to the values of the associated affine hyperplanes at the vertices of the $\dimx-1$ dimensional hypercube $[0,1]^{\dimx-1}$: $v_1$ is the “intercept” (the height of the hyperplane at the origin), $v_2$ is the height of the hypeplanehyperplane at vertex $(1 \quad 0 \cdots 0)^\prime\in\R^{\dimx-1}$ and so on. Nonetheless, the number of “compatibility conditions” grows much faster than the dimension. For instance, with $\dimx=4$ the functions $v_1,\dots, v_\dimx$ must be chosen in such a way that all of the mappings below are non-decreasing in $\tau$
\begin{align}
...
This exercise provides a very simple example of a stochastic process $\big((Y_t,Z_t)\colon t\ge0\big),$ where $Z_t=(W_t\quad W_tY_t)^\prime,$ satisfying the time homegeneous quantile regression equation \@ref(eq:QADLg). $\blacksquare$
 
```


When the functions $g_1,\dots,g_\lagmaxy$ are all equal to the identity function, the model described in equation \@ref(eq:QADLg) is precisely the QADL^[For *Quantile autoregressive distributed lag*.] model of @Galvao2013, up to the definition of $\mathfrak{F}_t$ and inclusion of a contemporaneous $Z_t$ (notice, however, that the “starting lag of $Z$” is arbitrary: we can always define $\tilde{X}_{t} = X_{t-1}$ in which case equation \@ref(eq:QADLg) includes a contemporaneous $\tilde{Z}_t$). If additionally the $\theta$’s are all equal to the zero function, then \@ref(eq:QADLg) corresponds to the QAR^[*Quantile autoregressive*, but you had already guessed this one.] model of @KoenkerXiao2006JASA. Some remarks: first, with this notation the QADL model as introduced by @Galvao2013 would have
...
```{example}
Assume that $Y_t|\mathfrak{F}_{t-1}$ has a Kumaraswamy distribution with parameters $Y_{t-1}/\alpha$ and $Y_{t-1}/\beta$, where $0<\alpha,\beta<1$. The quantile function of a random variable $V$ having the Kumaraswamy distribution with parameters $a>0$ and $b>0$ is given by
$$
...
@koenker2005 points out that, in a time series framework, quantile crossing is a more stringent restriction. Indeed, as I have insisted so far, in linear quantile regression models it is convenient to restrict any variables appearing in the right-hand side of the equation.
, since under strict stationarity if the quantile regression equation includes lagged values  the response variable
