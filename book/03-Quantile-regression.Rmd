...
An important property of the quantile regression model \@ref(eq:qr-model) is that the parameter $\beta$ appears as a solution to an optimization problem. This will be relevant later on when we are dealing with estimation.

```{theorem}
If \@ref(eq:qr-model) holds, then:
 
 1. $\E \rho_\tau\big(Y - X^\prime \beta(\tau)\big) \le \E\rho_\tau\big(Y - X^\prime b\big)$ for any $b\in\R^\dmax$ and $\tau\in(0,1)$.
 2. $\int_0^1\E\rho_\tau\big(Y - X^\prime \beta(\tau)\big)\,\dd\tau \le \int_0^1\E\rho_\tau\big(Y - X^\prime b(\tau)\big)\,\dd\tau$ for any measurable function $b\colon(0,1)\to\R^\dmax$.
...
Indeed, if the coefficient $\beta_2$ is not constant-in-$\tau$, then there exist two distinct quantile levels $\tau_{1}<\tau_{2}\in(0,1)$ such that either $\beta_2(\tau_1)<\beta_2(\tau_2)$ or $\beta_2(\tau_1)>\beta_2(\tau_2)$. In any case, if $\operatorname{support}(X_2)$ were unbounded from above and below, then there would exist an $x_2\in\operatorname{support}(X_2)$ such that, for $x = (1\quad x_2)^\prime$, one would have $Q_Y(\tau_1|x)>Q_Y(\tau_2|x)$ which is forbidden since $\tau\mapsto Q_Y(\tau|z)$ is non-decreasing.
